...
function insertRegionsForListing($listing_id, $region_id, $db)
{
    ...
    while ($region_id) {
        ...
        $db->Execute(
            "INSERT INTO `geodesic_listing_regions` SET `listing`=?, `region`=?, `level`=?, `primary_region`='yes', `default_name`=?",
...
function insertRegionsForUser($user_id, $region_id, $db)
{
    ...
    while ($region_id) {
        ...
        //insert for the region specified.
        $db->Execute(
            "INSERT INTO `geodesic_user_regions` SET `user`=?, `region`=?, `level`=?, `default_name`=?",
    ...
    if ($countries) {
        foreach ($countries as $country) {
            ...
            $result = $this->_db->Execute(
                "INSERT INTO `geodesic_region` SET `parent`=0, `level`=1, `enabled`='yes', `billing_abbreviation`=?,
...
if ($state_count < $state_total) {
    ...
    if ($states) {
        foreach ($states as $state) {
            ...
            $this->_db->Execute(
                "INSERT INTO `geodesic_region` SET `parent`=?, `level`={$level}, `enabled`='yes', `billing_abbreviation`=?,
					`unique_name`=?, `tax_percent`=?, `tax_flat`=?, `display_order`=?",
...
if ($region_addon_count < $region_addon_total) {
    ...
    if ($regions) {
        foreach ($regions as $region) {
            ...
            $this->_db->Execute(
                "INSERT INTO `geodesic_region` SET `parent`=?, `level`=?, `enabled`='yes',
