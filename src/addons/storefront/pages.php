...
class addon_storefront_pages extends addon_storefront_info
{
	...
	private function _displayListings ()
	{
		...
		if($sort_type == 43) $headers['type']['reorder'] = 44;
		elseif($sort_type == 44) $headers['type']['reorder'] = 0;
		else $headers['type']['reorder'] = 43;
		...
		if($sort_type == 5) $headers['title']['reorder'] = 6;
		elseif($sort_type == 6) $headers['title']['reorder'] = 0;
		...
		for ($i = 1; $i <= 20; $i++) {
			...
			if ($reg->$setting) {
				...
				$browse2 = $browse1 + 1;
				if($sort_type == $browse1) $headers['optionals'][$i]['reorder'] = $browse2;
				elseif($sort_type == $browse2) $headers['optionals'][$i]['reorder'] = 0;
		...
		if($sort_type == 7) $headers['city']['reorder'] = 8;
		elseif($sort_type == 8) $headers['city']['reorder'] = 0;
		...
		if($sort_type == 13) $headers['zip']['reorder'] = 14;
		elseif($sort_type == 14) $headers['zip']['reorder'] = 0;
		...
		if($sort_type == 1) $headers['price']['reorder'] = 2;
		elseif($sort_type == 2) $headers['price']['reorder'] = 0;
		...
		foreach($listingsA as $browse_type => $listingsOfType) {
			...
			if($sort_type == 4) $headers['entry_date']['reorder'] = 3;
			elseif($sort_type == 3) $headers['entry_date']['reorder'] = 0;
			...
			if($sort_type == 70) $headers['time_left']['reorder'] = 69;
			elseif($sort_type == 69) $headers['time_left']['reorder'] = 0;
