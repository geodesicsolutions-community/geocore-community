...
class addon_bulk_uploader_admin extends Admin_site {
	...
	public function display_addon_bulk_uploader_manage_uploads()
	{
		...
		$html .= '
		<div class="form-group">
			<label class="control-label col-md-5 col-sm-5 col-xs-12">Process a single file at once: '.geoHTML::showTooltip('Process a single file at once', 
					...
					ease the burden on the server when dealing with several update files that would otherwise update at the same time.').'</label>
			<div class="col-md-6 col-sm-6 col-xs-12">
	...
	public function update_addon_bulk_uploader_tokenize_images()
	{
		...
		if(move_uploaded_file($upload, $newFileName)){
			...
			if($stats['extant']) geoAdmin::m("Found {$stats['extant']} images with pre-existing tokens", geoAdmin::NOTICE);
			if($stats['recreate']) geoAdmin::m("Remade {$stats['recreate']} tokens with missing local images", geoAdmin::NOTICE);
			if($stats['new']) geoAdmin::m("Created {$stats['new']} new tokens", geoAdmin::NOTICE);
			if($stats['not_image']) geoAdmin::m("Detected {$stats['not_image']} URLs that were not valid image references", geoAdmin::NOTICE);
			if($stats['write_fail']) geoAdmin::m("Failed to write {$stats['write_fail']} images to local disk", geoAdmin::ERROR);
	 ...
	 public function insertCSV ($forceSettings=null, $forceHandle=null, $existingUids=null)
	 {
		...
		while (($values = fgetcsv($handle, 0, $delimiter, $encapsulation)) !== false) {
			...
			if(!$this->_isValidRow($values)) {
				if ($isCron) $cron->log('Values not good, not able to process row! Row is: '.print_r($values,1),__line__.$logAppend);
				continue;
			}
		...
		if (!$isCron) {
			...
			fclose($handle);
			//report stats
			if($insertCount) geoAdmin::m('Inserted '.$insertCount.' new listings');
			if($skipCount) geoAdmin::m('Skipped '.$skipCount.' listings. Either the "title" or "seller" data was missing, or you refreshed this page.', geoAdmin::NOTICE);
		} else {
