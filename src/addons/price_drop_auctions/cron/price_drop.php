...
foreach($result as $drop) {
	...
	if($settings['drop_amount_static'] == 1) {
		...
		$maxListingDuration = $listing->ends - $listing->date; //the maximal runtime of the auction, in seconds
		if($settings['delay_high'] == 0) {
			//special case to prevent division by zero: 5 minutes, to match Cron timer
			$settings['delay_high'] = 1/12;
		}
		$fewestNumDrops = $maxListingDuration / ($settings['delay_high']*3600); //price will drop AT LEAST this many times over the full duration
		...
		$dropAmt =  $drop['starting_price'] * ($dropPerc / 100); //we drop by a percentage of the STARTING (not current) price
