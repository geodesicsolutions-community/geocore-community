...
class addon_social_connect_util extends addon_social_connect_info
{
	...
	private function _fixUaForRedirect ()
	{
		...
		DataAccess::getInstance()->execute("UPDATE ".geoTables::session_table." SET `ip`='0', `ip_ssl`='0' WHERE `classified_session`=?",
			array($session_id));
	...
	public function userRegister ()
	{
		...
		if (geoMaster::is('classifieds')) array_push($plan_expirations_to_check,$user_info['price_plan_id']);
		if (geoMaster::is('auctions')) array_push($plan_expirations_to_check,$user_info['auction_price_plan_id']);
