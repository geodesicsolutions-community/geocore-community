...
<table class='table table-striped table-bordered table-hover'>
	...
	public function core_filter_display_page($full_text){
		...
		$full_text = preg_replace_callback('/href\s?=(\')([^\']+)(\')\n?./i', function($matches){return $this->formatUrls($matches[2],$matches[0]);}, $full_text);
		...
		$full_text = preg_replace_callback('/href=([^\'"\s>]+)(\s|>)/i', function($matches){return $this->formatUrls($matches[1],$matches[0]);}, $full_text);
	...
	public function rewriteUrl ($url, $url_encode_titles = false, $forceNoSSL = false)
	{
		...
		if (!$this->_templatePrefix && (strpos($url, self::$_url) === 0) || (strpos($url, self::$_secUrl) === 0)) {
			...
			} else if (!$this->_templatePrefix && strpos($url, geoFilter::getBaseHref()) === false) {
		...
		} else if (isset($get['a']) && $get['a'] == 2 && isset($get['b']) && $get['b'] !==0 && count($get) == 2) {
			//This is a listing URL...  with no domain part to it...
			$this->setSubdomain($get['b']);
		}
