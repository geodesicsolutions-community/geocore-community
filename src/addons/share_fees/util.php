...
class addon_share_fees_util {
	...
	public function checkUserToAttachment ($attaching_user_id=0, $user_attached_to_id=0) {
		...
		if ($attaching_user_group_id) {
			if ($this->attaching_user_group != $attaching_user_group_id) return false;
		} else {
			//no user group found
			return false;
		}
		...
		if ($user_attached_to_group_id) {
			if ($this->attached_to_user_group != $user_attached_to_group_id) return false;
		} else {
			//no user group found
			return false;
		}
	...
	public function removeUserAttachedTo ($user_id=0) {
		...
		if ($attachment_type_to_check == 0) {
			if ($this->attachment_type == 0) return false;
