...
{if !$steps_combined}<form action="{$process_form_url}" method="post">{/if}
	<div class="content_box">
		...
		{if !$display_description_last_in_form && ($editCheck || $fields->description->can_edit)}
			{include file="system/order_items/shared/details_description_box.tpl"}
		{/if}
		...
		{/if}
		{if $moreDetails}
			{include file='system/order_items/shared/listing_collect_details.tpl' more=$moreDetails}
		{/if}
		...
		{if $sell_type == 1 && $fields->price->is_enabled}
			...
			<div class="{if $error_msgs.price}field_error_row {/if}{cycle values='row_odd,row_even'}">
				...
				<div style="white-space: nowrap; display:inline;">
					...
					{include file="system/order_items/shared/postcurrency_dropdown.tpl"}
				</div>
					{else}
		...
		{if $sell_type == 2 && $editAuctionPrices && $fields->cost_options->is_enabled}
			<div class="{if $error_msgs.cost_options}field_error_row {/if}{cycle values='row_odd,row_even'}" id="cost_options_box_outer">
				...
				{include file='system/order_items/shared/cost_options/index.tpl'}
		...
		{if $moreDetailsPricing}
			{include file='system/order_items/shared/listing_collect_more.tpl' more=$moreDetailsPricing}
		{/if}
	...
	<div class="content_box">
		...
		{if $moreDetailsLocation}
			{include file='system/order_items/shared/listing_collect_more.tpl' more=$moreDetailsLocation}
		{/if}
...
{* capture this next bit into a smarty variable, so that it can be not shown if there's nothing to show *}
	{capture assign=additionalInfo}
		...
		{include file="system/order_items/shared/questions.tpl"}
...
{********************************************}
	...
	{if $moreDetailsEnd}
		...
		<br />
		<div class="content_box">
			{include file='system/order_items/shared/listing_collect_more.tpl' more=$moreDetailsEnd}
		</div>
	{/if}
