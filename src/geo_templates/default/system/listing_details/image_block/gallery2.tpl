			...
			{foreach $images as $image}
				...
				{else}
					<a href="get_image.php?id={$image.id}" class="big_link_{$classified_id}_{$image.id} lightUpLink mobile-lightbox-disable" onclick="return false;"{if !$image@first} style="display: none;"{/if}>
						<img src="{if $image.image_url}{$image.image_url}{else}{$image.url}{/if}"{if $image.scaled.large_gallery.width} style="width: {$image.scaled.large_gallery.width}px;"{/if} alt="{$image.image_text}" />
					</a>
				{/if}
			{/foreach}
		</div>
		...
		<ul>
			{foreach $images as $image}
				<li>
					...
					{else}
						<img class="thumb" src="{if $image.thumb_url}{$image.thumb_url}{else}{$image.url}{/if}"{if $image.scaled.small_gallery} style="width: {$image.scaled.small_gallery.width}px;"{/if} alt="{$image.image_text}" />
					{/if}
