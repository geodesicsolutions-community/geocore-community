...
class upgrade {
	...
	function _connectDB() {
 	...
 	function getUpgradeType() {
 		if($this->upgradeType!==null) return $this->upgradeType;
 	...
 	function getSelectSQL($oldTableName,$columnNames) {
 		$string = "SELECT ";
 		foreach($columnNames as $name) { $string .= $name.", "; }
 	...
 	function getUpdateSQL($newTableName,$columnNames,$values,$where=null) {
 		...
 		if($where!==null) { $string .= " WHERE ".$where[0]." = ".$where[1]; }
 	...
 	function getAssoc($values) {
 		...
 		$valuesFixed = array();
 		foreach($values as $key => $value) { if(!is_numeric($key)) { $valuesFixed[$key] = $value; } }
 		return $valuesFixed;
 	}
