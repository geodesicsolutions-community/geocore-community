...
<div class="lightUpTitle" id="newConfirmTitle">Move Value(s)</div>

<form style="display:block; margin: 15px; width: 450px;" action="index.php?page=leveled_field_value_move&amp;leveled_field={$leveled_field}&amp;parent={$parent}&amp;p={$page}" method="post">
	{foreach $values as $value_id}
		<input type="hidden" name="values[]" value="{$value_id}" />
	{/foreach}
	<p class="page_note">
		<strong>Warning:</strong> This can take some time if you are moving a lot of values,
		or are moving values that have a lot of sub-values.  Select where to move the values to below:
	</p>
	...
	<div class="{cycle values='row_color1,row_color2'}">
		<div class="leftColumn">Move TO Subvalues Of</div>
		<div class="rightColumn">
	...
	<div id="moveToBrowseBox" style="display: none;">
		{include file='leveled_fields/moveBrowse.tpl'}
	</div>
	<br /><br />
