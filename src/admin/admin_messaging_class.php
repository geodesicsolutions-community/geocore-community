...
class Admin_messaging extends Admin_site
{
    ...
    function admin_messaging_form($db = 0, $list_info = 0, $message_id = 0)
    {
        ...
        if (!$this->admin_demo()) {
            $this->body .= "<div class=\"center\"><input type=submit name='auto_save' value=\"Send\"></div>";
        }
        $this->body .= "</table>\n\t</td>\n</tr>\n";
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function get_last_ten_messages_list($db)
    {
        ...
        if (!$past_messages_result) {
            ...
            $this->body .= "</select>";
            if (!$this->admin_demo()) {
                $this->body .= "<span class='input-group-btn'><input type=submit name=c[message_type] class=\"btn btn-primary\" value=\"Select Recent Message\" style=\"margin: 0 !important; font-size: 14px;\"></span>
				</div>
			  </div>
			";
            }
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function get_form_messages_list($db)
    {
        ...
        if (!$form_messages_result) {
            ...
            $this->body .= "</select>";
            if (!$this->admin_demo()) {
                $this->body .= "<span class='input-group-btn'><input type=submit name=c[message_type] class=\"btn btn-primary\" value=\"Select Form Message\" style=\"margin: 0 !important; font-size: 14px;\"></span>
				</div>
			  </div>";
            } else {
    ...
    function send_admin_message($db, $list_info = 0, $message_info = 0, $start_from = 0, $message_id = 0)
    {
        ...
        if ($list_info && ($message_info || $message_id)) {
            ...
            if (!$message_id) {
                ...
                if (
                    (strlen(trim($message_info["subject"])) > 0) &&
