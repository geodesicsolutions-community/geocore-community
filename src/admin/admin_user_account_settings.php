...
class admin_user_account_settings extends Admin_site
{
	...
	public function update_user_account_settings()
	{
		...
		if(!$this->db->set_site_setting('user_rating_low_threshold', $threshold)) return false;
		if(!$this->db->set_site_setting('user_rating_low_notify_user', ((isset($settings['user_rating_low_notify_user'])&&$settings['user_rating_low_notify_user'])? 1 : false) )) return false;
		if(!$this->db->set_site_setting('user_rating_low_notify_admin', ((isset($settings['user_rating_low_notify_admin'])&&$settings['user_rating_low_notify_admin'])? 1 : false) )) return false;
