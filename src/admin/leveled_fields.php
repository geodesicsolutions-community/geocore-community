...
class LeveledFieldsManage
{
    ...
    public function update_leveled_field_edit()
    {
        ...
        $label = geoString::toDB($label);
        $db->Execute(
            "UPDATE " . geoTables::leveled_fields . " SET `label`=? WHERE `id`=?",
    ...
    public function update_leveled_field_value_create()
    {
        ...
        //insert it in there
        $result = $db->Execute(
            "INSERT INTO " . geoTables::leveled_field_value . " SET `leveled_field`=?, `parent`=?, `level`=?, `enabled`=?, `display_order`=?",
    ...
    public function update_leveled_field_value_edit()
    {
        ...
        //insert it in there
        $result = $db->Execute(
            "UPDATE " . geoTables::leveled_field_value . " SET `level`=?, `enabled`=?, `display_order`=?
				WHERE `id`=?",
    ...
    private function copyValue($value_id, $leveled_field, $parent)
    {
        ...
        foreach ($langs as $lang) {
            $row = $db->GetRow(
                "SELECT * FROM " . geoTables::leveled_field_value_languages . " WHERE `id`=? AND `language_id`=?",
            ...
            $db->Execute(
                "INSERT INTO " . geoTables::leveled_field_value_languages . " SET `id`=?, `language_id`=?, `name`=?",
