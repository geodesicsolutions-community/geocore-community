...
class Ad_configuration extends Admin_site
{
    ...
    public function update_listing_photo_upload_settings()
    {
        ...
        if ($oldNewSettings['image_upload_path'] && !is_dir($oldNewSettings['image_upload_path'])) {
            ...
            geoAdmin::m(
                'Warning: The specified image upload path <span style="white-space: nowrap;">&quot;' . geoString::specialChars($oldNewSettings['image_upload_path']) . '&quot;</span>
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function set_sitewide_auto_title($useMe, $choices)
    {
        if ($useMe != 1) {
            ...
            if (!$this->db->set_site_setting("use_sitewide_auto_title", 1)) {
                return false;
            }
            if (!$this->db->set_site_setting("sitewide_auto_title", $title)) {
                return false;
            }
        }
            return true;
        ...
        if (!$result) {
            ...
            if (!$result) {
                ...
                if (geoPC::is_ent()) {
                    ...
                    if ($show_site_configuration["voting_system"] == 1) {
                        $this->body .= "selected";
                    }
                    $this->body .= ">IP based discrimination</option>";
                    $this->body .= "<option value=2 ";
                    if ($show_site_configuration["voting_system"] == 2) {
                        $this->body .= "selected";
                    }
                    $this->body .= ">User based</option>";
                    $this->body .= "<option value=3 ";
                    if ($show_site_configuration["voting_system"] == 3) {
                        $this->body .= "selected";
                    }
    ...
    function ad_extras_form()
    {
        ...
        if (!$result) {
            ...
            if ($interval >= $day && $interval % $day == 0) {
                $timeUnit = $day;
            } elseif ($interval >= $hour && $interval % $hour == 0) {
                $timeUnit = $hour;
            } elseif ($interval >= $minute && $interval % $minute == 0) {
                $timeUnit = $minute;
            }
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function payment_types_form($db)
    {
        ...
        if (!$type_result) {
            ...
            if (!$this->admin_demo()) {
                $this->body .= "<form action=index.php?mc=listing_setup&page=listing_payment_types method=post>\n";
            }
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    public function update_listing_bid_increments()
    {
        ...
        foreach ($deleting as $low) {
            $this->db->Execute(
                "DELETE FROM " . geoTables::increments_table . " WHERE `low`=? LIMIT 1",
                array($low . '')
            );
        }
        if (count($deleting)) {
        ...
        if (isset($_POST['addLow']) && isset($_POST['addIncrement'])) {
            ...
            if ($low >= 0.00 && $increment > 0.00) {
                ...
                if ($count == 0) {
                    //insert it
                    $this->db->Execute(
                        "INSERT INTO " . geoTables::increments_table . " (`low`,`increment`) VALUES (?, ?)",
                        array ($low, $increment)
                    );
