...
//###########################################################
    ...
    public function display_classified($id = 0, $return = false, $preview = false, $autoDisplay = true, $session_vars = null, $printFriendly = false)
    {
        ...
        } elseif (geoPC::is_print() && $this->db->get_site_setting('disableAllBrowsing')) {
        ...
        if ($this->debug_ad_display_time) {
            echo $this->get_end_time() . " after template gotten<br/>\n";
        }
        ...
        if ($this->debug_ad_display_time) {
            echo $this->get_end_time() . " after live check<br/>\n";
        }
        ...
        if ($this->debug_ad_display_time) {
            echo $this->get_end_time() . " after lead picture placed<br/>\n";
        }
        ...
        if ($this->debug_ad_display_time) {
            echo $this->get_end_time() . " after most fields placed<br/>\n";
        }
        ...
        if (geoMaster::is('auctions') && $seller_data) {
            ...
            if (!$printFriendly) {
                $view->seller_rating_scale_explanation = $this->display_help_link(102826);
            }
        }
        ...
        if ($show["item_type"] == 2) {
            ...
            if ($show["reserve_price"] != 0.00 && !$show["buy_now_only"]) {
                ...
                if (!$reverse_auction && $show["current_bid"] >= $show["reserve_price"]) {
                    //reserve met
                    $reserve_met = true;
                } elseif ($reverse_auction && $show["current_bid"] <= $show["reserve_price"] && $show["current_bid"] != 0.00) {
            ...
            if ($show["auction_type"] != 2 && $show['buy_now'] > 0) {
                ...
                if ($show['buy_now_only']) {
                    ...
                    $show_buy_now = true;
                } elseif ($show['current_bid'] == 0) {
                    //there are no bids yet, so show buy now option
                    $show_buy_now = true;
                } elseif ($show['current_bid'] != 0 && $this->configuration_data['buy_now_reserve'] == 1 && $show['reserve_price'] > 0 && !$reserve_met) {
            ...
            if ($this->debug_ad_display_time) {
                echo $this->get_end_time() . " after bidding info placed<br/>\n";
            }
            ...
            if ($this->debug_ad_display_time) {
                echo $this->get_end_time() . " after pricing placed<br/>\n";
            }
        }
        ...
        if ($this->debug_ad_display_time) {
            echo $this->get_end_time() . " after seller info placed<br/>\n";
        }

        if (!$printFriendly) {
            if ($this->debug_ad_display_time) {
                echo $this->get_end_time() . " after images and info placed<br/>\n";
            }
            for ($i = 1; $i <= 20; $i++) {
