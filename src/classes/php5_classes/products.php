...
final class geoPC
{
	 ...
	 * @return geoPC
	 */
	public static final function getInstance ($type=null)
	{
	...
	private function validate ($forceRemote = false, $addon = '')
	{
		...
		if ($geo->errors) { return $geo->errors; }
...
agreement;
	...
	private function validate_local_key ($local_key, $forceRemote=false, $addon='', $recurse=false)
	{
		...
		if ($geo->save_local_key||$forceRemote) $raw_local_key = $local_key;
		...
		} else if (defined('IN_ADMIN')&&(string)$key_data['local_key_expires']!='never'&&(integer)$key_data['local_key_expires']<time()) {
		...
		if ($this->validate_download_access&&(integer)$key_data['download_access_expires']<$geo->release_date && !($key_data['custom_fields']['leased'] && !$key_data['download_access_expires'])) {
	...
	public function db_read_local_key ($forceRemote=false, $addon='')
	{
		...
		} else if (!$forceRemote&&$geo->classType=='geoPC') {
		...
		if (!$local_key || strpos($local_key,'<')!==false) {
			...
			if ($local_key) $geo->save_local_key=true;
		}
	...
	{
		$fp=@fopen($path, 'w');
		if (!$fp) { return $this -> errors="Error: I could not save the local license key."; }
		@fwrite($fp, $local_key);
		@fclose($fp);
	...
	private function _mainDemo ()
	{
		...
		if (!defined('IN_ADMIN')) {
			...
			foreach ($valid_tsets as $tset) {
				...
				} else if (strpos($tset, '_secondary')!==false) {
	...
	public static function clearLicenseKey ($onlyClearData = false, $addon='')
	{
		...
		if ($addon) {
			$geo = $this->add[$addon];
			if (!$onlyClearData) $geo->reg->set($geo->prefix.'license_key',false);
			...
			if (!$onlyClearData) $db->set_site_setting('license',false);
	...
	public static function urlGetContents ($url, $timeout=0, $additionalHeaders=null)
	{
		...
		} else if (defined('GEO_CURL_CAINFO') && strpos(GEO_CURL_CAINFO, '.pem')!==false) {
...
class geoSession {
	...
	public function getIpField(){
		return (self::isSSL())? 'ip_ssl' : 'ip';		return 'ip_ssl';
	}
	/**
	 * Internal
	 * @internal
...
status=new');
			if (!isset(self::$status)) self::$status = 'new';
...
status=error (db error)');
			if (!isset(self::$status)) self::$status = 'error';
			return false;
		}
...
status=changed (record count for session is wrong)');
			if (!isset(self::$status)) self::$status = 'changed';
			return ($this->_newSession());
		}
...
database is not valid.  Here is the data returned from the database:<br>'.print_r($credentials,1));
		...
		if (!isset(self::$status)) self::$status = 'confirmed';
	...
	{
		if (!isset($this->languageId)) $this->setLanguage();
