<?php
/**
 * Encoded file, used for license validation, database access, and other
 * core-level functionality.
 *
 * @package System
 */

//no common header, make it harder to decode.
...
class DataAccess {
	...
	public function get_site_settings($return_table = false){
		...
		if (!$use_cache || $config_data === false){
			...
			if ($use_cache) $cacheSettings->update('configuration_data',$this->configuration_data);
		} else {
			$this->configuration_data = $config_data;
		}
		if ($use_cache) $site_settings = $cacheSettings->process('site_settings');
		if (!$use_cache || $site_settings === false){
			...
			if(false === $rows) {
		 	...
		 	foreach ($rows as $row) {
		 		...
		 		if ($use_cache) $cacheAdd[$row['setting']] = $row['value'];
		 	}
		 	if ($use_cache) $cacheSettings->update('site_settings',$cacheAdd);
		} else {
	...
	public function get_text($return_text = false, $current_page_id=0)
	{
		...
		if ($cache_on) $textCache = geoCacheText::getInstance();
		$language_id = $this->getLanguage();
		$use_cache = true;
		foreach ($args_list as $page_id) {
			if (!isset($this->messages_pages[$page_id])) {
				if ($cache_on) $cacheTextArray = $textCache->process($language_id, $page_id);
				if ($cache_on) trigger_error('DEBUG STATS: cache process: $language_id = '.$language_id.' page_id = '.$page_id);
		...
		if (!$cache_on || !$use_cache){
			...
			foreach ($result as $show) {
				...
				if ($cache_on) $cacheArray[$show['page_id']][$show['text_id']] = $message;
			}
	...
	public function num_new_ads_in_category($category_id, $ends_limit, $placed_limit){
		if (!isset($this->preloaded_num_new_ads)) $this->preload_num_new_ads($ends_limit,$placed_limit);
	...
	public function moduleTag ($tag, $params, $smarty)
	{
		...
		if ($cacheResult === false){
			...
			if (!$show_module){
				...
				if (geoCache::get('cache_module')) $moduleCache->update($tag, $show_module);
			}
	...
	public function checkBannedIp()
	{
		...
		if ($ip_result->RecordCount() > 0) {
			...
			if ($ban_me){
				...
				if (!defined('IS_ROBOT')) define ('IS_ROBOT',1);
			}
		}
	...
	public function addBrowsingWhereClause ($name, $sql, $data = null, $additionalTable=null)
	{
		...
		if ($data!==null) {
			...
			foreach($data as $arr) {
				...
				if (isset($sqlarr[$i])) {
					$sql .= $sqlarr[$i];
					if ($i+1 != sizeof($sqlarr)) $this->db->outp_throw( "Input Array does not match ?: ".htmlspecialchars($sql),'Execute');
