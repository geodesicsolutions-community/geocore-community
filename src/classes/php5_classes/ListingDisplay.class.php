 ...
 */
    ...
    public static function mapping_direction_link_google(geoListing $listing, $params, $smarty)
    {
        ...
        return geoTemplate::loadInternalTemplate(
            $params,
            $smarty,
            'mapping_directions_link_google.tpl',
            geoTemplate::SYSTEM,
            'listing_details',
            $tpl_vars
        );
    ...
    public static function show_ad_vote_comments_link(geoListing $listing, $params, $smarty)
    {
        ...
        return geoTemplate::loadInternalTemplate(
            $params,
            $smarty,
            'show_ad_vote_comments_link.tpl',
            geoTemplate::SYSTEM,
            'listing_details',
            $tpl_vars
        );
    ...
    public static function seller_rating(geoListing $listing, $params, $smarty)
    {
        ...
        } elseif (($seller->feedback_score == 0) || ($seller->feedback_count == 0)) {
            $sql = "select filename from " . geoTables::auctions_feedback_icons_table . " where begin = 0";
            $tpl_vars['score_image'] = trim($db->GetOne($sql));
        } elseif ($seller->feedback_score < 0) {
    ...
    public static function multi_level_field_breadcrumb(geoListing $listing, $params, $smarty)
    {
        ...
        return geoTemplate::loadInternalTemplate(
            $params,
            $smarty,
            'multi_level_field_breadcrumb.tpl',
            geoTemplate::SYSTEM,
            'listing_details',
            $tpl_vars
        );
            ...
            if (!$user_id) {
                ...
                if ($configuration_data['invited_list_of_buyers'] && $configuration_data['black_list_of_buyers']) {
                    ...
                    if ($is_invited == 1) {
                        ...
                        $canBid = true;
                    } elseif (!$is_banned && $invited_count == 0) {
                        //browsing user is not on any list, but invited list is not
                        //populated so allow them
                        $canBid = true;
                    }
                } elseif ($configuration_data['invited_list_of_buyers']) {
                    //check invited only
                ...
                } elseif ($configuration_data['black_list_of_buyers']) {
    ...
    private static function _voteSummary($listing)
    {
        if (!isset($listing->e_voteSummary)) {
            ...
            if ($totalVotes == 0) {
                ...
                } elseif ($listing->three_votes > $listing->two_votes && $listing->three_votes > $listing->one_votes) {
