...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function list_current_ads()
    {
        ...
        foreach ($listings as $key => $listing) {
            ...
            if ($listing['item_type'] == 1) {
                ...
                if ($listing['price_applies'] == 'item') {
                    $bids = $this->db->Execute(
                        "SELECT * FROM " . geoTables::bid_table . " WHERE `auction_id`=? AND `buy_now_bid`=1 ORDER BY `time_of_bid` DESC",
                        array($listing['id'])
                    );
            ...
            if ($this->db->get_site_setting('days_can_upgrade') && geoPC::is_ent()) {
                ...
                if (($this->db->get_site_setting('days_can_upgrade') && (geoUtil::time() < $upgrade_cutoff))) {
                    if (
                        ($this->db->get_site_setting('use_bolding_feature') && $listing['bolding'] == 0 && $pricePlan['use_bolding']) ||
        ...
        foreach ($all as $row) {
            ...
            } elseif ($item->get('renew_upgrade') == 2) {
