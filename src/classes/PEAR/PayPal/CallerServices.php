...
class CallerServices extends SOAP_Client
{
    ...
    function CallerServices($profile)
    {
        ...
        if(defined('GEO_CURL_CAINFO')) {
        	//GEO MOD: use GEO_CURL_CAINFO if it is set (see config.php for more info)
        	//			if it is not set, skip this so that the server uses its own defaults
        	$this->setOpt('curl', CURLOPT_CAINFO, GEO_CURL_CAINFO);
        }
        //original: $this->setOpt('curl', CURLOPT_CAINFO, dirname(__FILE__) . '/cert/api_cert_chain.crt');
        //	the above references a certificate that expired in 2011, and is probably worthless in most cases
