...
class tempSiteClass extends geoSite
{
        ...
        if (is_array($info)) {
            ...
            if ($this->session_variables["buy_now_only"]) {
                ...
                if (isset($info['price_applies']) && $info['price_applies'] == 'item') {
                    $price_applies = 'item';
                } elseif (!isset($info['price_applies']) && isset($this->session_variables['price_applies']) && $this->session_variables['price_applies'] == 'item') {
                    $price_applies = 'item';
                }
            } else {
        ...
        if ($num_questions > 0) {
            foreach ($this->session_variables['question_value'] as $key => $value) {
                if (strlen(trim($value)) > 0) {
                    ...
                    } elseif ($this->category_choices[$key] == 'date') {
    ...
    function classified_detail_check($db = 0, $category_id = 0, $edit = false)
    {
        ...
        if ($this->fields->title->is_enabled) {
            ...
            if ($this->debug) {
                echo $this->session_variables["classified_title"] . " after stripslashes - " . strlen($this->session_variables["classified_title"]) . "<br />\n";
            }
            ...
            if ($this->debug) {
                echo $this->session_variables["classified_title"] . " after maxlength - " . strlen($this->session_variables["classified_title"]) . "<br />\n";
            }
        ...
        if ($this->fields->zip->is_enabled) {
            ...
            if ($this->fields->zip->is_required) {
                ...
                } elseif ($this->fields->zip->text_length && strlen(trim($this->session_variables["zip_code"])) > $this->fields->zip->text_length) {
                    //zip too long
                    $this->error++;
                    $this->error_variables["zip_code"] = "error";
                }
        ...
        if ($this->sell_type == 2) {
            ...
            if ($this->session_variables["buy_now_only"]) {
                ...
                } elseif (strlen(trim($this->session_variables["auction_minimum"])) > 0) {
            ...
            for ($i = 1; $i < 21; $i++) {
                ...
                if ($field->is_enabled) {
                    ...
                    if ($field->field_type == 'number' || $field->field_type == 'cost') {
                        ...
                        } elseif ($field->field_type == 'cost' && $field->is_required && geoNumber::deformat($this->session_variables[$fieldName]) == 0) {
                            //it is required and adds cost and they entered 0, silly peoples...
                            $this->error++;
                            $this->error_variables[$fieldName] = "error";
                        } elseif (strlen($this->session_variables[$fieldName]) > 0) {
