        ...
        return{
            ...
            div:function (id,parent,append) {
                ...
                if (!parent || parent == 'body') {
                    ...
                    if ( !parent ) {
                        parent = CJAX.is_element(parent,false);
                    }
                }
                if ( !parent ) {
                    return false;
    ...
    this.set                =           function () {
        return {
            ...
            ,type: function (elem,new_type,verbose) {
                ...
                var elem = CJAX.is_element(elem,verbose);
                if ( elem ) {
                    elem.type = new_type; return true;}
                return false;
            }
            ,name : function (elem,new_name,verbose) {
                ...
                var elem = CJAX.is_element(elem,verbose);
                if ( elem ) {
                    elem.name = new_name; return true;}
                return false;
...
/*alert( e );*/}
    ...
    this.exe_html       =       function ( params ) {
        ...
        CJAX.HTTP_REQUEST_INSTANCE.onreadystatechange       =       function () {
            if (CJAX.HTTP_REQUEST_INSTANCE.readyState < 4) {
                ...
                if ( !image ) {
                    image = "cjax/core/images/loading.gif'";
                }
                if ( element ) {
                    element.innerHTML = "<span><img src='" + image + "/>&nbsp;" + text + "</div>";
                }
            }
            try {
        ...
        if (this.mode.toLowerCase()  == "get") {
            ...
            CJAX.HTTP_REQUEST_INSTANCE.onreadystatechange       =       function () {
                if (CJAX.HTTP_REQUEST_INSTANCE.readyState < 4) {
                    ...
                    if ( obj ) {
                        obj.innerHTML = "<span>" + this.image + "&nbsp;" + this.text + "</div>";
                    }
                }
    ...
    */
        ...
        return {
            ...
            action : function (f,p,s) {
                try {
                    ...
                    alert('unabled to load function: ' + f.toUpperCase() + ' ' + e); }
            }
        }
    }();
        ...
        if ( ext ) {
            ...
            if (typeof eval(f) === 'function' ) {
                try {
                    ...
                    alert('unabled to load function: ' + CJAX.method.toUpperCase() + ' ' + e); }
            }
        }
    }
        ...
        for ( var i = 0; i < sheets.length; i++ ) {
            ...
            try {
                do {
                    ...
                    if (this.rule.selectorText.toLowerCase() == rule) {
                        return this.rule; }
                }                 while ( cssRule ) {
                }
            } catch ( e ) {
    ...
    this.create_element     =       function ( buffer ) {
        ...
        if (_class) {
            newelem.setAttribute('class',_class);
        }
        newelem.setAttribute('id',elem);
        if (_html) {
            newelem.innerHTML = _html;
        }
    ...
    this.is_element         =           function (element,verbose) {
        ...
        if ( verbose ) {
            if ( !element ) {
                alert('invalid input on function: ' + _FUNCTION + ' :  not an element ');return;}
        }
        var _element;
    ...
    this.form_get_elements_url      =       function ( frm_object ) {
        ...
        for (var n = 0; n < form.length; n++) {
            if (form[n].id == 'undefined' && form[n].name == 'undefined') {
                continue;
            }
            c++;
            elem = form[n];
            ...
            if (value != 'undefined' && elem_id) {
                frm_url += "&" + elem_id + "=" + value;
            }
        }
        return frm_url;
    }
    ...
    this.exe_form       =       function ( params ) {
        ...
        if (frm != '') {
            ...
            if ( !form ) {
                ...
                if ( !url ) {
                    alert('CJAX: Please specify a form name'); return false; }
            } else {
                        ...
                        case 'radio':
                            if (CJAX.getSelectedRadio(elem) != -1) {
                                if (CJAX.getSelectedRadioValue(elem)) {
                                    elem_value = CJAX.getSelectedRadioValue(elem);
                                } assign = '=';
                            } else {
        ...
        CJAX.HTTP_REQUEST_INSTANCE.onreadystatechange = function () {
            if (CJAX.HTTP_REQUEST_INSTANCE.readyState <= 4) {
                ...
                if ( container ) {
                    container.innerHTML = "<span>" + image + "&nbsp;" + text + "</span>";
                }
            }
            if (CJAX.HTTP_REQUEST_INSTANCE.readyState == 4 && CJAX.HTTP_REQUEST_INSTANCE.status == 200) {
                var txt =  unescape(CJAX.HTTP_REQUEST_INSTANCE.responseText);
    ...
    this.elem_doc       =       function (id_obj,verbose) {
        var type = (typeof elem);
        if ( typeof verbose == 'undefined' && CJAX.debug) {
            verbose = true; }
        if ( type.indexOf('object') == -1) {
        ...
        if ( !elem ) {
            if ( verbose ) {
                alert('CJAX: Element "' + id_obj + '" not found on document');
            }
            return false;
        }
        return elem;
    ...
    {
        if (CJAX.script.load(__base__ + "/components/extensions.js")) {
            CJAX.COMPONENTS['extensions'] = 1;
        }
