...
class CoreEvents
{
    ...
    public function __call($method, $args)
    {
        ...
        if (!$this->extension_dir) {
            $this->extension_dir = '__base__/extensions/';
        }
    ...
    function call($url, $element_id = '', $cmode = "get")
    {
        ...
        if ($this->text) {
            $out[] = "<text>{$this->text}</text>";
        }
        if ($this->image) {
            $out[] = "<image>{$this->image}</image>";
        }
        if ($cmode != 'get') {
            $out[] = "<mode>{$cmode}</mode>";
        }
    ...
    function load($url, $elem_id, $mode = 'get')
    {
        ...
        if ($this->image) {
            $out[] = "<image>{$this->image_dir}{$this->image}</image>";
        }
        if ($this->debug) {
            $out[] = "<dubug>1</debug>";
        }
    ...
    function form($url, $form_id, $container_id = null)
    {
        ...
        $out[] = "<url>$url</url>";
        if ($form_id) {
            $out[] = "<form>$form_id</form>";
        }
        if (!is_null($container_id)) {
            $out[] = "<container>$container_id</container>";
        }
        if ($this->text) {
            $out[] = "<text>{$this->text}</text>";
        }
        ...
        if ($this->method && $this->method != 'get'/* get is default, so it is not a priority */) {
            $out[] = "<method>$this->method</method>";
        }
        if (isset($out) && is_array($out)) {
            $params  = implode($out);
        }
