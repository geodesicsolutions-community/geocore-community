...
function registration($db, $product, &$template, $errors = 0, $config = 0)
{
    ...
    $template = str_replace(
        "(!REGISTRATION_EMAIL_DESCRIPTION!)",
        "Email address that will receive registration confirmation and success messages sent to admin.",
        $template
    );
    ...
    $template = str_replace(
        ...
        "Email address that will be used to send out all emails from the server.",
        $template
    );
    ...
    $template = str_replace(
        "(!REGISTER_URL_DESCRIPTION!)",
        "The register.php can be placed anywhere you like within your software's distribution files.",
        $template
    );
    ...
    if ($errors == 1) {
        echo "Please enter a valid filename or none.  If you feel this is in error please leave it blank and set it in
            the admin after install in the registration configuration section.";
    }
...
{
    $sql = 'REPLACE INTO `geodesic_site_settings` (`setting`,`value`) VALUES (\'site_email\', \''
        . $config['site_email'] . '\'), ( \'registration_admin_email\', \''
        . $config['admin_registration_email'] . '\')';
    ...
    $sql_query = "update " . $product['config_table'] . " set " . $product['url_register'] . " = \""
        . $config['url_register'] . "\"";
    //echo $sql_query.'<br>';
    $result = $db->Execute($sql_query);
    if (!$result) {
